 ...
 * - 호출 시점: 신청 승인 API 등 인원 변동이 일어나는 시점에 다른 팀원(2번 팀원 등)이 호출해서 사용함
 * ====================================================================
 */
...
export const getMyNotifications = async (req, res, next) => {
    try {
        ...
        // 프론트엔드 기획서 명세서(235.jpg / 555.jpg) 구조 규격에 100% 맞춰 응답 패킷 전송
        return res.status(200).json({ 
            success: true,                       // 성공 여부 판단 플래그 (기획서 필수 항목)
            message: "알림 목록을 불러왔습니다.", // 명세서 사진과 글자 하나 틀리지 않고 완벽 매칭시킨 성공 문구
            data: {
...
// - 용도: 프론트 연동 전 포스트맨으로 백엔드 로직이 올바르게 굴러가는지 단독 검증/테스트용 라우터
// ==========================================
export const triggerMatchComplete = async (req, res, next) => {
    try {
        // 클라이언트가 요청 본문(Body)에 담아 보낸 JSON 데이터에서 post_id 추출
        const { post_id } = req.body;

        // 예외 검증: Body 데이터에 post_id 인자값이 비어 있거나 누락되었다면 400 에러 처리
        if (!post_id) {
            return res.status(400).json({ 
                success: false,
                message: "post_id 값이 필요합니다." 
            });
        }

        // 파일 상단에 구현해 둔 매칭 완료 공용 비즈니스 함수를 호출하고 연산 결과 상태 코드를 반환받음
        const result = await sendMatchCompleteNotification(post_id);

        // [예외 상황 분기 1]: 인원이 부족하여 알림이 가지 않은 경우
        if (result.status === 'NOT_FULL') {
            return res.status(200).json({ 
                success: false, // 실제 전송이 일어난 상황은 아니므로 false 처리하여 프론트에 인지시킴
                message: result.message 
            });
        }
        
        // [예외 상황 분기 2]: 아예 없는 방이거나(404), 이미 마감이 종결된 방(400)일 경우의 동적 에러 응답
        ...
        // [최종 성공 분기]: 모든 유효성 검사를 마치고 방장/참가자 전원 알림 및 방 마감 처리가 깔끔하게 완료된 경우
        return res.status(200).json({ 
            success: true,
            message: result.message 
        });
        ...
        console.error("매칭 완료 알림 발송 에러:", error); // 콘솔에 에러 기록
        next(error);                                    // 에러 미들웨어로 처리 이관
    }
};